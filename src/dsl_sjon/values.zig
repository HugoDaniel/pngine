...
//!
//! Positional sub-forms (`(module code)`, `(vertex …)`, `(entry …)`) are read off
...
pub const Reader = struct {
    ...
    /// First positional (non-kvpair) child node of a form, or null. Used to read
    /// single-positional sub-forms like `(module code)` or `(topology …)`.
    ...
    /// Symbol text of the first positional of `form` (e.g. `(module code)` → "code").
    ...
    /// Read a literal number-typed key (author number or defaulted number). Does
    /// NOT evaluate expressions — use `evalU32` for expression-capable slots.
    ...
    /// Read an integer-typed key as u32 (literal only).
    pub fn u32Of(self: Reader, form: NodeIndex, key: []const u8) ?u32 {
        const n = self.number(form, key) orelse return null;
        if (n < 0) return null;
    ...
    /// through `evalU32`, so `:dispatch-workgroups (ceil (/ N 64))` works, while
    /// every vector element used to read through `elemNumber`, which takes number
    /// nodes and nothing else. `:dispatch [(ceil (/ N 64)) 1 1]` therefore
    ...
    /// vector slots (dispatch, viewport, scissor, origin, pool offsets). Negative
    /// results clamp to 0 — these are all unsigned wire fields.
    pub fn elemU32(
        self: Reader,
        gpa: std.mem.Allocator,
        schema: Schema.Schema,
    ...
    /// `:dispatch-workgroups (ceil (/ NUM 64))`, bare define `NUM`, or a literal)
...
/// Narrow error set for expression evaluation — SJON's broad `Expr.Error`
/// (type mismatches, unknown bindings, arity, …) collapses to `EmitError`.
pub const EvalError = error{ OutOfMemory, EmitError };
...
// define-ref reaches these three sites unchecked. They used to clamp the low
// end (`@max(0, n)`) and nothing else, so the high end was a panic in the CLI
// and a trap-or-wrap in the ReleaseSmall wasm compiler (LEAK-11 C). Every slot
// that accepts an expression funnels through here: `:size`, draw and dispatch
// counts, viewport/scissor/origin.
//
// Refusal is `EmitError`; the caller with a source node in hand turns it into a
// located diagnostic.

fn narrowF64(n: f64) EvalError!u32 {
    // Written as `!(n <= max)` so NaN — which compares false to everything —
    // takes the refusal branch rather than the cast.
    if (!(n <= @as(f64, std.math.maxInt(u32)))) return error.EmitError;
    return @intFromFloat(@max(0, n));
}

fn narrowI64(i: i64) EvalError!u32 {
    if (i > std.math.maxInt(u32)) return error.EmitError;
    return @intCast(@max(0, i));
}

fn narrowU64(u: u64) EvalError!u32 {
    if (u > std.math.maxInt(u32)) return error.EmitError;
    return @intCast(u);
}
...
/// Evaluate an arbitrary value node to u32 against `env`.
pub fn evalNodeU32(
    gpa: std.mem.Allocator,
    tree: *const Ast.Tree,
    schema: Schema.Schema,
...
/// Map a `filter-mode` symbol to its enum (default `linear`, as the legacy does).
pub fn mapFilterMode(sym: ?[]const u8) FilterMode {
    if (sym) |s| if (std.mem.eql(u8, s, "nearest")) return .nearest;
    return .linear;
}
...
/// `view-dimension` is authored as an int 0..5 (digit-leading enums can't be SJON
/// symbols — §227); the BGL descriptor JSON carries the WebGPU string so gpu.js can
/// pass it straight to createBindGroupLayout. Out-of-range → "2d" (the WebGPU default).
