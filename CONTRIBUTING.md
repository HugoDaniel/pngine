...
### Runtime JS Source Manifests (Important)

When adding or splitting files under `npm/pngine/src/`, remember there are two
source-copy manifests in `build.zig` that must be updated together:

1. `js_files` for `zig build web` (`zig-out/playground/*`)
2. `website_js_files` + `runtime_files` for `zig build website` (`website/assets` and `website/public`)

If a new runtime module is imported (for example command-handler splits from
`gpu.js`) but not listed in these manifests, source-mode builds will fail at
runtime with missing module errors even though `npm run build` may still pass.
...
### 14. NPM Runtime Profile Split (Viewer/Dev/Core/Executor)

The browser package now exposes profile-specific entrypoints:

- `pngine` / `pngine/viewer`: lean production player for PNG payloads with embedded executor.
- `pngine/dev`: full browser feature surface (selectors, image init, shared fallback paths).
- `pngine/core`: low-level dispatcher API.
- `pngine/executor`: advanced payload/executor helpers.

Bundle outputs are generated by `npm --prefix npm/pngine run build`:
...
Viewer API contract intentionally keeps a narrow production surface:

- `pngine(source, { canvas, debug?, onError? })`
- `source`: URL string or bytes (`Uint8Array` / `ArrayBuffer` / `Blob`)
- `canvas` is required
- no `wasmUrl`, selector sources, or `HTMLImageElement` sources in viewer

Worker/runtime split is also profile-specific:

- `src/viewer-init.js` -> `src/worker-viewer.js` (embedded executor only)
- `src/init.js` -> `src/worker.js` (embedded + shared `wasmUrl` fallback)

This split should be preserved when adding new runtime features: decide whether a
feature belongs in viewer (production path) or dev (full compatibility path) before
adding it to public exports.
