...
fn runViewer(allocator: std.mem.Allocator, opts: Options) !void {
    ...
    if (opts.output_path) |_| {
        // No doc path here: this prints to end users, and the one it used to
        // cite moved to docs/plans/archive/ — which the release mirror strips
        // anyway. Point at the command that does render instead.
        std.debug.print("\nNote: this viewer traces command buffers; it does not render.\n", .{});
        std.debug.print("To render a frame: pngine <input> --frame -o out.png\n", .{});
    }
}
